...
Status BlockBasedTable::ReadRangeDelBlock(
  ...
  if (!s.ok()) {
    ...
    std::unique_ptr<InternalIterator> iter(NewDataBlockIterator<DataBlockIter>(
        ...
        /*get_context=*/nullptr, lookup_context, Status(), prefetch_buffer,false,true));
    assert(iter != nullptr);
    s = iter->status();
    if (!s.ok()) {
      ROCKS_LOG_WARN(
...
Status BlockBasedTable::ReadMetaIndexBlock(
  ...
  Status s = ReadBlockFromFile(
      ...
      rep_->blocks_definitely_zstd_compressed, nullptr /* filter_policy */,true);
...
TBlockIter* BlockBasedTable::NewDataBlockIterator(
    ...
    FilePrefetchBuffer* prefetch_buffer, bool for_compaction,bool is_meta_block) const {
